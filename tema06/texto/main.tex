...
\begin{center}
  ...
  \vspace{0.2cm}
  {\normalsize CIC1205 -- Machine Learning\par}

  \vfill

  {\Large\bfseries Trabalho 3 -- Tema 6\par}
  \vspace{1.4cm}
  {\LARGE\bfseries Pseudo-rotulagem fraca para reconhecimento de entidades\par}
  \vspace{0.35cm}
  {\LARGE\bfseries em relatos do Disque Denúncia\par}
  ...
  \begin{minipage}{0.82\textwidth}
    \raggedright
    \normalsize
    \textbf{Alunos:} Jose Antonio Ferreira e Ricardo Araujo\\[0.5cm]
    \textbf{Professor:} Eduardo Bezerra\\[0.5cm]
    Relatório acadêmico apresentado na disciplina
    \textbf{CIC1205 -- Machine Learning} como parte dos requisitos de
    avaliação do Trabalho 3.
  \end{minipage}

  \vfill

  {\large Rio de Janeiro\par}
  \vspace{0.15cm}
...
\citep{melo2026ren}, que investigou o problema de REN em relatos informais do
serviço Disque Denúncia do Rio de Janeiro. Nessa dissertação, o objetivo central
foi identificar automaticamente entidades dos tipos Pessoa, Localização e
Organização em relatos criminais escritos em linguagem coloquial, com erros
ortográficos e estrutura textual livre. Além do texto dos relatos, a dissertação
também considerou o uso de metadados estruturados como fonte adicional de
contexto para o problema. O corpus anotado utilizado, chamado DDsmall, contém
5.230 relatos anotados manualmente.
...
pseudo-rotulagem em uma abordagem de ajuste fino com modelos de linguagem,
em particular o GLiNER, que avalia o contexto ao redor de cada
...
dissertação de Gustavo Melo \citep{melo2026ren}:
...
Do ponto de vista das ferramentas do spaCy, as Etapas 1 e 2 não utilizaram
componentes da biblioteca: a extração, a normalização e a filtragem do léxico
foram implementadas com estruturas de dados em Python sobre os spans já
anotados do corpus. O spaCy passou a ser usado apenas a partir da projeção
lexical, na tokenização, no casamento de padrões e, depois, no treinamento e
na execução dos pipelines de reconhecimento de entidades.
...
Na implementação, esse matcher foi construído sobre um pipeline mínimo para
português criado com \texttt{spacy.blank('pt')}, usado apenas para fornecer
tokenização e vocabulário ao \texttt{PhraseMatcher}. Assim, o spaCy não foi
empregado aqui como modelo estatístico, mas como infraestrutura linguística
leve para segmentação em tokens e casamento eficiente de expressões.
...
Em termos de submódulos, o treinamento utilizou principalmente
\texttt{spacy.training.Example} para converter anotações em exemplos
supervisionados, \texttt{nlp.add\_pipe('ner')} para adicionar o reconhecedor de
entidades ao pipeline, \texttt{nlp.initialize(...)} para inicializar os pesos,
\texttt{spacy.util.minibatch} para formar os lotes de treinamento e
\texttt{nlp.evaluate(...)} para acompanhar o desempenho no conjunto de
validação. Quando disponível, a execução foi configurada com
\texttt{spacy.prefer\_gpu()} para permitir uso de GPU.
...
\begin{description}
  \item[Baseline 1 (zero-shot)] O modelo pré-treinado \texttt{pt\_core\_news\_lg}
    ...
    DDsmall (Pessoa, Localização, Organização). Neste experimento, foi usado o
    pipeline pré-treinado completo carregado com \texttt{spacy.load}, sem
    reentreinamento nem ajuste adicional ao domínio dos relatos.
  ...
  \item[Proposta 1 (EntityRuler)] Sistema baseado puramente em regras, construído
    ...
    insensível a maiúsculas. O componente foi instanciado em um pipeline
    \texttt{spacy.blank('pt')} com configuração \texttt{phrase\_matcher\_attr = LOWER},
    e os padrões foram ordenados do mais longo para o mais curto.
  ...
  \item[Baseline 2 (DDsmall)] Modelo NER treinado exclusivamente com os 4.226
    ...
    com a anotação manual disponível, sem dados adicionais. Sua implementação
    usou um pipeline \texttt{spacy.blank('pt')} com o componente treinável
    \texttt{ner}.
  ...
  \item[Proposta 2 (20k pseudo)] Modelo NER treinado com os 4.226 relatos do
    ...
    Assim como o Baseline 2, foi treinado do zero em spaCy com
    \texttt{spacy.blank('pt')} e componente \texttt{ner}, mudando apenas o
    conjunto de exemplos usados no treinamento.
  ...
  \item[Proposta 2 (completo)] Variante que utilizou todos os 133.841 relatos
    ...
    A arquitetura e a biblioteca utilizadas foram as mesmas da Proposta 2 (20k),
    variando somente a quantidade de pseudo-exemplos incluídos.
\end{description}
...
macro, a média simples entre as três classes. Para os experimentos com modelos
do spaCy, as predições foram obtidas executando os pipelines carregados com
\texttt{spacy.load(...)} sobre os relatos do conjunto de teste; no caso do
Baseline 1, houve ainda o mapeamento dos rótulos nativos do modelo pré-treinado
para o esquema de classes adotado neste trabalho.
